...
export function HanziClient({
  ...
  const [sentences, setSentences] =
    useState<SentenceWithProgress[]>(initialSentences);
  ...
  const [activeSentenceIds, setActiveSentenceIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  // Sentence game state
  const [sentenceChineseItems, setSentenceChineseItems] = useState<
    SentenceLinkItem[]
  >([]);
  const [sentencePinyinItems, setSentencePinyinItems] = useState<
    SentenceLinkItem[]
  >([]);
  const [sentenceEnglishItems, setSentenceEnglishItems] = useState<
    SentenceLinkItem[]
  >([]);
  const [sentenceConnections, setSentenceConnections] = useState<
    SentenceConnection[]
  >([]);
  const [selectedSentenceItem, setSelectedSentenceItem] =
    useState<SentenceLinkItem | null>(null);

  // Flash state for showing correct/incorrect feedback
  const [flashingIds, setFlashingIds] = useState<Map<string, boolean>>(
    new Map(),
  );

  // New item overlay state
  const [newWordOverlay, setNewWordOverlay] = useState<Word | null>(null);
  const [newSentenceOverlay, setNewSentenceOverlay] = useState<Sentence | null>(
    null,
  );
  ...
  const [selectedWordType, setSelectedWordType] = useState<WordType | "all">(
    "all",
  );
  ...
  const [resetNotification, setResetNotification] = useState<string | null>(
    null,
  );
  ...
  const initializeGame = useCallback(() => {
    ...
    const { englishItems, pinyinItems, hanziItems } =
      prepareRoundData(selected);
  ...
  const prepareSentenceRoundData = useCallback(
    (selectedSentences: SentenceWithProgress[]) => {
      ...
      const chineseItems: SentenceLinkItem[] = shuffle(
      ...
      const pinyinItems: SentenceLinkItem[] = shuffle(
        selectedSentences.map((s) => ({
          id: `pinyin-${s.id}`,
          sentenceId: s.id,
          content: s.pinyin,
      ...
      const englishItems: SentenceLinkItem[] = shuffle(
        selectedSentences.map((s) => ({
          id: `english-${s.id}`,
          sentenceId: s.id,
          content: s.english,
  ...
  const initializeSentenceGame = useCallback(() => {
    ...
    const { chineseItems, pinyinItems, englishItems } =
      prepareSentenceRoundData(selected);
  ...
  const updateWordProgress = useCallback(
    async (wordId: string, wasCorrect: boolean) => {
  ...
  const checkDivergenceAndReset = useCallback(() => {
    ...
    if (currentDivergence.shouldReset) {
      const reason: ResetReason =
        currentDivergence.delta > 0 ? "too-hard" : "too-easy";
  ...
  const handleCompleteConnection = useCallback(
    async (connection: Connection) => {
      ...
      const englishItem = englishItems.find(
        (i) => i.id === connection.englishId,
      );
      ...
      const wordId =
        englishItem?.wordId || pinyinItem?.wordId || hanziItem?.wordId;

      // Set flash state for all three items
      const itemIds = [
        connection.englishId,
        connection.pinyinId,
        connection.hanziId,
  ...
  const handleItemSelect = useCallback(
    (item: LinkItem) => {
      ...
      const englishItem =
        selectedItem.type === "english"
          ? selectedItem
          : item.type === "english"
            ? item
            : null;
      const pinyinItem =
        selectedItem.type === "pinyin"
          ? selectedItem
          : item.type === "pinyin"
            ? item
            : null;
      const hanziItem =
        selectedItem.type === "hanzi"
          ? selectedItem
          : item.type === "hanzi"
            ? item
            : null;
      ...
      if (existingConnection) {
        ...
        updated.isComplete =
          updated.englishId !== null &&
          updated.pinyinId !== null &&
          updated.hanziId !== null;
  ...
  const updateSentenceProgress = useCallback(
    async (sentenceId: string, wasCorrect: boolean) => {
        ...
        if (sentence.progress) {
          await supabase
            ...
            .update({
              ...
              correct_streak: wasCorrect
                ? sentence.progress.correct_streak + 1
                : 0,
              last_seen: new Date().toISOString(),
              updated_at: new Date().toISOString(),
            })
  ...
  const replaceSentence = useCallback(
    (sentenceId: string) => {
      // Remove the old items
      setSentenceChineseItems((prev) =>
        prev.filter((i) => i.sentenceId !== sentenceId),
      );
      setSentencePinyinItems((prev) =>
        prev.filter((i) => i.sentenceId !== sentenceId),
      );
      setSentenceEnglishItems((prev) =>
        prev.filter((i) => i.sentenceId !== sentenceId),
      );
      ...
      const availableSentences = sentences.filter(
        (s) =>
          !activeSentenceIds.has(s.id) && !recentlyCompleted.includes(s.id),
      );
  ...
  const handleCompleteSentenceConnection = useCallback(
    async (connection: SentenceConnection) => {
      const chineseItem = sentenceChineseItems.find(
        (i) => i.id === connection.chineseId,
      );
      const pinyinItem = sentencePinyinItems.find(
        (i) => i.id === connection.pinyinId,
      );
      const englishItem = sentenceEnglishItems.find(
        (i) => i.id === connection.englishId,
      );
      ...
      const sentenceId =
        chineseItem?.sentenceId ||
        pinyinItem?.sentenceId ||
        englishItem?.sentenceId;

      // Set flash state
      const itemIds = [
        connection.chineseId,
        connection.pinyinId,
        connection.englishId,
      ].filter(Boolean) as string[];
      setFlashingIds((prev) => {
      ...
      setSentenceConnections((prev) =>
        prev.map((c) =>
          c.chineseId === connection.chineseId ? { ...c, isCorrect } : c,
        ),
      );
    ...
    [
      sentenceChineseItems,
      sentencePinyinItems,
      sentenceEnglishItems,
  ...
  const handleSentenceItemSelect = useCallback(
    (item: SentenceLinkItem) => {
      ...
      // Determine which items we're working with
      const chineseItem =
        selectedSentenceItem.type === "chinese"
          ? selectedSentenceItem
          : item.type === "chinese"
            ? item
            : null;
      const pinyinItem =
        selectedSentenceItem.type === "pinyin"
          ? selectedSentenceItem
          : item.type === "pinyin"
            ? item
            : null;
      const englishItem =
        selectedSentenceItem.type === "english"
          ? selectedSentenceItem
          : item.type === "english"
            ? item
            : null;
    ...
    [
      ...
      sentenceConnections,
      flashingIds,
      handleCompleteSentenceConnection,
    ],
  );
      ...
      <div className="flex items-center justify-between py-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-sm font-medium ${sessionScore >= 0 ? "text-emerald-400" : "text-red-400"}`}
            >
              {sessionScore > 0 ? "+" : ""}
              {sessionScore}
            </span>
          ...
          <div className="flex items-center justify-between text-neutral-400">
            <span>
              ...
              <span className="text-neutral-200">
                {boardDifficultyResult.score.toFixed(1)}
              </span>
            </span>
            <span>
              Expected:{" "}
              <span className="text-neutral-200">
                {expectedDifficulty.toFixed(1)}
              </span>
            </span>
            <span>
              Delta:{" "}
      ...
      {newWordOverlay && (
        ...
        >
          <div className="text-center">
            ...
            <div
              className="mt-6 space-y-2 animate-hanzi-reveal"
              style={{ animationDelay: "200ms" }}
            >
              <div className="text-2xl text-neutral-300">
                {newWordOverlay.pinyin}
              </div>
              <div className="text-lg text-neutral-500">
                {newWordOverlay.english}
              </div>
            </div>
            <div
              className="mt-8 text-sm text-neutral-600 animate-hanzi-reveal"
      ...
      {newSentenceOverlay && (
        ...
        >
          <div className="text-center px-8 max-w-lg">
            ...
            <div
              className="mt-6 space-y-3 animate-hanzi-reveal"
              style={{ animationDelay: "200ms" }}
            >
              <div className="text-xl text-neutral-300">
                {newSentenceOverlay.pinyin}
              </div>
              <div className="text-lg text-neutral-500">
                {newSentenceOverlay.english}
              </div>
            </div>
            <div
              className="mt-8 text-sm text-neutral-600 animate-hanzi-reveal"
        ...
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-40 animate-overlay-in">
          ...
          >
            <span className="text-sm">
              {tierChange.type === "promoted" ? "↑" : "↓"} {tierChange.from} →{" "}
              {tierChange.to}
            </span>
          </div>
        </div>
