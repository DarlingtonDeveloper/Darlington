...
export function WeekClient({ initialDate }: WeekClientProps) {
  ...
  const fetchWeekData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/calendar/events?date=${dateStr}&range=week`,
      );
  ...
  useEffect(() => {
    fetchWeekData(); // eslint-disable-line react-hooks/set-state-in-effect -- fetch on mount
  }, [fetchWeekData]);
  ...
  const busiestDay = days.reduce(
    (max, day) => (day.meetingMinutes > (max?.meetingMinutes || 0) ? day : max),
  ...
  const generateBusyBar = (events: CalendarEvent[]) => {
    ...
    events.forEach((e) => {
      ...
      for (
        let h = Math.max(9, startHour);
        h < Math.min(18, endHour);
        h += 0.5
      ) {
      ...
      {!loading && days.length > 0 && (
        <div className="space-y-2">
          {days.map((day) => {
            ...
            const isBusiest =
              busiestDay &&
              day.date === busiestDay.date &&
              day.meetingMinutes > 120;
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-sm font-medium ${isToday ? "text-emerald-400" : "text-neutral-400"}`}
                    >
                      {day.dayName}
                    </span>
                    <span
                      className={`text-sm font-semibold ${isToday ? "text-neutral-100" : "text-neutral-300"}`}
                    >
                      {day.dayNumber}
                    </span>
                    {isToday && (
