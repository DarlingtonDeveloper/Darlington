...
import {
  PartialComplete,
  usePartialComplete,
} from "@/components/habits/partial-complete";
...
export function HabitsClient({
  ...
  const [todaysFocusHabits, setTodaysFocusHabits] = useState<string[]>(
    initialData.focusHabitIds,
  );
  ...
  const loadHabitsForDate = useCallback(
    async (date: Date) => {
      setIsLoading(true);
      try {
        ...
        const [completionsResult, stepsResult, stepCompletionsResult] =
          await Promise.all([
            supabase
            ...
            supabase.from("habit_steps").select("*").order("display_order"),
            supabase
        ...
        const habitsWithStatus: HabitWithStatus[] = (habitsData || []).map(
          (habit) => {
            ...
            return {
              ...
              completion_percentage:
                completion?.completion_percentage ?? (completion ? 100 : 0),
              notes: completion?.notes ?? null,
              steps: habitSteps.length > 0 ? habitSteps : undefined,
              completedStepIds:
                habitCompletedStepIds.length > 0
                  ? habitCompletedStepIds
                  : undefined,
            };
          },
        );
  ...
  async function toggleStep(
    ...
    try {
      if (currentlyCompleted) {
        ...
        setHabits(
          habits.map((h) =>
            h.id === habitId
              ? {
                  ...h,
                  completedStepIds: (h.completedStepIds || []).filter(
                    (id) => id !== stepId,
                  ),
                }
              : h,
        ...
        const { error } = await supabase.from("habit_step_completions").insert({
          step_id: stepId,
          user_id: userId,
          completion_date: dateString,
        });
        ...
        const allStepsDone = habit.steps.every((s) =>
          newCompletedStepIds.includes(s.id),
        );
  ...
  async function completeAllSteps(habitId: string) {
    ...
    const incompleteSteps = habit.steps.filter(
      (s) => !(habit.completedStepIds || []).includes(s.id),
    );
    ...
    try {
      ...
      setHabits(
        habits.map((h) =>
          h.id === habitId ? { ...h, completedStepIds: allStepIds } : h,
        ),
      );
  ...
  const percentage =
    totalCount > 0 ? Math.round((completionSum / totalCount) * 100) : 0;
  ...
  return (
    <div
      {...swipeHandlers}
      className="min-h-full bg-neutral-950 text-neutral-50"
    >
      {/* Header with date navigator */}
      <div className="bg-neutral-950 border-b border-neutral-800">
        <div className="px-4 sm:px-6 py-4 sm:max-w-2xl sm:mx-auto">
            ...
            return (
              <div key={categoryKey} className="mb-6 sm:mb-4">
                <div className="flex items-center justify-between mb-3 sm:mb-2">
                  <h2
                    className={`text-xs font-medium uppercase tracking-wide ${canEdit ? "text-neutral-500" : "text-neutral-600"}`}
                  >
                    {categoryKey}
                  </h2>
                  <span
                    className={`font-mono text-xs tabular-nums ${canEdit ? "text-neutral-600" : "text-neutral-700"}`}
                  >
                    ...
                    habit.steps && habit.steps.length > 0 ? (
                      ...
                      <HabitSteps
                        ...
                        steps={habit.steps.map((s) => ({
                          ...s,
                          completed: (habit.completedStepIds || []).includes(
                            s.id,
                          ),
                        }))}
                        ...
                        onToggleStep={(stepId, completed) =>
                          toggleStep(stepId, habit.id, completed)
                        }
                      ...
                      // Regular habit - use button with focus border
                      <FocusBorder
                        key={habit.id}
                        active={
                          viewingIsToday &&
                          todaysFocusHabits.includes(habit.id) &&
                          canEdit
                        }
                      >
                        <button
                        ...
                        >
                          <div className="flex items-center gap-4 sm:gap-3">
                            ...
                            <div className="flex-1 flex items-center justify-between min-w-0">
                              ...
                              {habit.health_link ? (
                                <span
                                  className={`flex items-center gap-1.5 font-mono text-sm sm:text-xs tabular-nums ml-3 flex-shrink-0 ${habit.health_auto_completed ? "text-emerald-500" : "text-neutral-500"}`}
                                >
                                  <svg
                                ...
                                habit.completed_at && (
                                  <span
                                    className={`font-mono text-sm sm:text-xs tabular-nums ml-3 flex-shrink-0 ${canEdit ? "text-neutral-500" : "text-neutral-600"}`}
                                  >
        ...
        <div className="px-4 sm:px-6 pb-safe sm:max-w-2xl sm:mx-auto">
          <div className="bg-neutral-900/50 border border-neutral-800/50 rounded-lg p-4 text-center">
            <span className="text-sm text-neutral-500">
              This day is read-only. You can only edit today or yesterday
              (within 24 hours).
            </span>
          </div>
        </div>
      ...
      <PartialComplete
        ...
        currentPercentage={
          habits.find((h) => h.id === targetHabitId)?.completion_percentage ?? 0
        }
