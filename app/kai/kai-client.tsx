 ...
 */
export function KaiClient({ starters }: KaiClientProps = {}) {
  ...
  const extractText = useCallback(
    (msg: Record<string, unknown>): string | null => {
  ...
  const connect = useCallback(async () => {
    ...
    ws.onclose = (event) => {
      ...
      if (event.code === 1008 || event.code === 4001 || event.code === 4008) {
        setError(
          `Connection rejected (${event.code}): ${event.reason || "auth failed"}`,
        );
        return;
      }
    ...
    return (
      <div className="flex flex-col h-full items-center justify-center px-8 text-center">
        ...
        <p className="text-sm mb-4" style={{ color: "var(--fg2, #6b6560)" }}>
          You need to be logged in to chat with Kai.
        </p>
        <a
          href="/login"
  ...
  return (
    <div className="flex flex-col h-full max-w-2xl mx-auto">
      ...
      {connState === "disconnected" && (
        <div
          className="text-center py-2 text-xs"
          style={{ color: "var(--accent2, #a89880)" }}
        >
