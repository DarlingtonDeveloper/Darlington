...
function getTimeLeft() {
  ...
  const yearProgress = Math.min(
    1,
    Math.max(0, (now - YEAR_START) / (YEAR_END - YEAR_START)),
  );
...
export function CountdownTimer() {
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      ...
      {iterations === null ? (
        <div className="h-[1.1rem] w-28 mt-1 rounded bg-white/5 animate-pulse" />
      ) : (
        <span
          className="font-mono text-[0.8rem] tracking-wide text-[var(--fg)] mt-1"
          style={{ textShadow: "0 0 8px var(--accent)" }}
        >
          <span className="tabular-nums">{iterations.toLocaleString()}</span>
          <span className="text-[var(--fg2)] ml-1">iterations left</span>
        </span>
      )}
    </div>
  );
